package snippets

import (
	"bytes"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
	"k8s.io/gengo/generator"
	"k8s.io/gengo/types"
)

func TestGenerateSetterForType(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	testStruct := newTestType(t, "SomeStruct")

	tests := []struct {
		description     string
		pointerReceiver bool
		want            string
	}{
		{
			description:     "use value receiver",
			pointerReceiver: false,
			want: `// WithName is an autogenerated function
func (o SomeStruct) WithName(in string) SomeStruct {
	o.ObjectMeta.Name = in
	return o
}

`,
		},
		{
			description:     "use pointer receiver",
			pointerReceiver: true,
			want: `// WithName is an autogenerated function
func (o *SomeStruct) WithName(in string) *SomeStruct {
	o.ObjectMeta.Name = in
	return o
}

`,
		},
	}

	for _, test := range tests {
		parent := getMemberFromType(t, testStruct, "SomeStruct", "ObjectMeta").Type
		member := getMemberFromType(t, testStruct, "SomeStruct", "ObjectMeta", "Name")

		var b bytes.Buffer
		setter := NewSetter(testStruct, parent, test.pointerReceiver)
		sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
		sw.Do(setter.GenerateSetterForType(member))
		assert.NoError(t, sw.Error())
		assert.Equal(t, test.want, b.String())
	}
}

func TestGenerateSetterForBool(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	testStruct := newTestType(t, "SomeStruct")
	parent := getMemberFromType(t, testStruct, "SomeStruct").Type
	member := getMemberFromType(t, testStruct, "SomeStruct", "Bool")

	want := `// WithBool is an autogenerated function
func (o *SomeStruct) WithBool(in ...bool) *SomeStruct {
	o.SomeStruct.Bool = variadicBool(in...)
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(testStruct, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForBool(member))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForPointerToBool(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	testStruct := newTestType(t, "SomeStruct")
	parent := getMemberFromType(t, testStruct, "SomeStruct").Type
	member := getMemberFromType(t, testStruct, "SomeStruct", "PointerBool")

	want := `// WithPointerBool is an autogenerated function
func (o *SomeStruct) WithPointerBool(in ...bool) *SomeStruct {
	o.SomeStruct.PointerBool = boolPointer(variadicBool(in...))
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(testStruct, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForPointerToBool(member))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForMap(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct").Type

	tests := []struct {
		description string
		member      types.Member
		want        string
	}{
		{
			description: "setter for map[int]string",
			member:      getMemberFromType(t, someStruct, "SomeStruct", "MapIntString"),
			want: `// WithMapIntString is an autogenerated function
func (o *SomeStruct) WithMapIntString(in map[int]string) *SomeStruct {
	if o.SomeStruct.MapIntString == nil {
		o.SomeStruct.MapIntString = make(map[int]string)
	}
	for key, value := range in {
		o.SomeStruct.MapIntString[key] = value
	}
	return o
}

`,
		},
		{
			description: "setter for map[string][]byte",
			member:      getMemberFromType(t, someStruct, "SomeStruct", "MapStringByteSlice"),
			want: `// WithMapStringByteSlice is an autogenerated function
func (o *SomeStruct) WithMapStringByteSlice(in map[string][]byte) *SomeStruct {
	if o.SomeStruct.MapStringByteSlice == nil {
		o.SomeStruct.MapStringByteSlice = make(map[string][]byte)
	}
	for key, value := range in {
		o.SomeStruct.MapStringByteSlice[key] = value
	}
	return o
}

`,
		},
	}

	for _, test := range tests {
		var b bytes.Buffer
		setter := NewSetter(root, parent, true)
		sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
		sw.Do(setter.GenerateSetterForMap(test.member))
		assert.NoError(t, sw.Error())
		assert.Equal(t, test.want, b.String())
	}
}

func TestGenerateSetterForMapStringString(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")

	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct", "ObjectMeta").Type
	member := getMemberFromType(t, someStruct, "SomeStruct", "ObjectMeta", "Labels")
	want := `// WithLabels is an autogenerated function
func (o *SomeStruct) WithLabels(in map[string]string) *SomeStruct {
	o.ObjectMeta.Labels = mergeMapStringString(o.ObjectMeta.Labels, in)
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForMapStringString(member))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForMemberSlice(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")

	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct").Type

	tests := []struct {
		description string
		member      types.Member
		want        string
	}{
		{
			description: "Setter for String Slice",
			member:      getMemberFromType(t, someStruct, "SomeStruct", "Strings"),
			want: `// AppendStrings is an autogenerated function
func (o *SomeStruct) AppendStrings(in ...string) *SomeStruct {
	o.SomeStruct.Strings = append(o.SomeStruct.Strings, in...)
	return o
}

`,
		},
		{
			description: "Setter for Byte Slice",
			member:      getMemberFromType(t, someStruct, "SomeStruct", "Bytes"),
			want: `// WithBytes is an autogenerated function
func (o *SomeStruct) WithBytes(in []byte) *SomeStruct {
	o.SomeStruct.Bytes = in
	return o
}

`,
		},
	}

	for _, test := range tests {
		var b bytes.Buffer
		setter := NewSetter(root, parent, true)
		sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
		sw.Do(setter.GenerateSetterForMemberSlice(test.member))
		assert.NoError(t, sw.Error())
		assert.Equal(t, test.want, b.String())
	}
}

func TestGenerateSetterForEmbeddedSlice(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	cStruct := newTestType(t, "CStruct")

	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct").Type
	member := getMemberFromType(t, someStruct, "SomeStruct", "CStructs")
	inputType := cStruct
	want := `// AppendCStructs is an autogenerated function
func (o *SomeStruct) AppendCStructs(in ...*CStruct) *SomeStruct {
	for _, elem := range in {
		if elem != nil {
			o.SomeStruct.CStructs = append(o.SomeStruct.CStructs, elem.CStruct)
		}
	}
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForEmbeddedSlice(member, inputType))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForMemberStruct(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")

	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct").Type
	member := getMemberFromType(t, someStruct, "SomeStruct", "AStruct")
	want := `// WithAStruct is an autogenerated function
func (o *SomeStruct) WithAStruct(in *a.AStruct) *SomeStruct {
	if in != nil {
		o.SomeStruct.AStruct = *in
	}
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForMemberStruct(member))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForEmbeddedStruct(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	cStruct := newTestType(t, "CStruct")

	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct").Type
	member := getMemberFromType(t, someStruct, "SomeStruct", "CStruct")
	inputType := cStruct
	want := `// WithCStruct is an autogenerated function
func (o *SomeStruct) WithCStruct(in *CStruct) *SomeStruct {
	if in != nil {
		o.SomeStruct.CStruct = in.CStruct
	}
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForEmbeddedStruct(member, inputType))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForPointerToBuiltinType(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")

	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct").Type
	member := getMemberFromType(t, someStruct, "SomeStruct", "IntPtr")
	want := `// WithIntPtr is an autogenerated function
func (o *SomeStruct) WithIntPtr(in int) *SomeStruct {
	o.SomeStruct.IntPtr = &in
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForPointerToBuiltinType(member))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForEmbeddedPointer(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	cStruct := newTestType(t, "CStruct")

	root := someStruct
	parent := getMemberFromType(t, someStruct, "SomeStruct").Type
	member := getMemberFromType(t, someStruct, "SomeStruct", "PointerCStruct")
	want := `// WithPointerCStruct is an autogenerated function
func (o *SomeStruct) WithPointerCStruct(in *CStruct) *SomeStruct {
	if in != nil {
		o.SomeStruct.PointerCStruct = &in.CStruct
	}
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForEmbeddedPointer(member, cStruct))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestFuncName(t *testing.T) {
	t.Parallel()

	tests := []struct {
		description string
		name        string
		kind        types.Kind
		elem        *types.Type
		want        string
	}{
		{
			description: "Setter for Builtin type",
			name:        "Name",
			kind:        types.Builtin,
			want:        "WithName",
		},
		{
			description: "Setter for Struct type",
			name:        "Struct",
			kind:        types.Struct,
			want:        "WithStruct",
		},
		{
			description: "Setter for String Slice type",
			name:        "StringSlice",
			kind:        types.Slice,
			elem:        types.String,
			want:        "AppendStringSlice",
		},
		{
			description: "Setter for Byte Slice type",
			name:        "ByteSlice",
			kind:        types.Slice,
			elem:        types.Byte,
			want:        "WithByteSlice",
		},
	}

	for _, test := range tests {
		m := types.Member{
			Name: test.name,
			Type: &types.Type{
				Kind: test.kind,
				Elem: test.elem,
			},
		}

		assert.Equal(t, test.want, funcName(m), test.description)
	}
}

func TestGenerateSetterForAliasPointerPrimitive(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	root := someStruct
	member := getMemberFromType(t, someStruct, "SomeStruct", "Alias")
	want := `// WithAlias is an autogenerated function
func (o *SomeStruct) WithAlias(in *b.AliasOfString) *SomeStruct {
	p := b.AliasOfString(in)
	o.Alias = &p
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, member.Type, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForAliasPointerPrimitive(member, member.Type))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForStringEnum(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	anEnumType := newTestType(t, "AnEnum")
	root := someStruct
	member := getMemberFromType(t, someStruct, "SomeStruct", "AnEnum")
	parent := getMemberFromType(t, someStruct, "SomeStruct")
	want := `// WithAnEnum is an autogenerated function
func (o *SomeStruct) WithAnEnum(in AnEnum) *SomeStruct {
	o.SomeStruct.AnEnum = b.AliasOfString(in)
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent.Type, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForTypeEnum(member, anEnumType))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())
}

func TestGenerateSetterForEmbeddedSliceEnum(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	anEnumType := newTestType(t, "AnEnum")
	root := someStruct
	member := getMemberFromType(t, someStruct, "SomeStruct", "ManyEnums")
	parent := getMemberFromType(t, someStruct, "SomeStruct")

	want := `// AppendManyEnums is an autogenerated function
func (o *SomeStruct) AppendManyEnums(in ...AnEnum) *SomeStruct {
	for _, elem := range in {
		o.SomeStruct.ManyEnums = append(o.SomeStruct.ManyEnums,  b.AliasOfString(elem))
	}
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent.Type, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForEmbeddedSliceEnum(member, anEnumType))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())

}

func TestGenerateSetterForEmbeddedSlicePointer(t *testing.T) {
	t.Parallel()

	ctx, err := newTestGeneratorContext()
	require.NoError(t, err)

	someStruct := newTestType(t, "SomeStruct")
	argType := newTestType(t, "CStruct")
	root := someStruct
	member := getMemberFromType(t, someStruct, "SomeStruct", "ManyPointers")
	parent := getMemberFromType(t, someStruct, "SomeStruct")

	want := `// AppendManyPointers is an autogenerated function
func (o *SomeStruct) AppendManyPointers(in ...*CStruct) *SomeStruct {
	for _, elem := range in {
		if elem != nil {
			o.SomeStruct.ManyPointers = append(o.SomeStruct.ManyPointers, &elem.CStruct)
		}
	}
	return o
}

`
	var b bytes.Buffer
	setter := NewSetter(root, parent.Type, true)
	sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
	sw.Do(setter.GenerateSetterForEmbeddedSlicePointer(member, argType))
	assert.NoError(t, sw.Error())
	assert.Equal(t, want, b.String())

}

func TestMemberAccessor(t *testing.T) {
	t.Parallel()

	someStruct := newTestType(t, "SomeStruct")

	tests := []struct {
		description string
		parent      *types.Type
		member      types.Member
		want        string
	}{
		{
			description: "Root and Parent are the same, access member directly",
			parent:      someStruct,
			member:      getMemberFromType(t, someStruct, "SomeStruct", "AStruct"),
			want:        "AStruct",
		},
		{
			description: "Root and Parent are different, need intermediary to access member",
			parent:      getMemberFromType(t, someStruct, "SomeStruct", "ObjectMeta").Type,
			member:      getMemberFromType(t, someStruct, "SomeStruct", "ObjectMeta", "Name"),
			want:        "ObjectMeta.Name",
		},
	}

	for _, test := range tests {
		setter := NewSetter(someStruct, test.parent, true)
		assert.Equal(t, test.want, setter.memberAccessor(test.member), test.description)
	}
}
