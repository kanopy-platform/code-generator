...
`,
	...
	for _, test := range tests {
		var b bytes.Buffer
		sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
		sw.Do(GenerateSetterForEmbeddedSlice(test.root, test.parent, test.member, test.wrapper))
		assert.NoError(t, sw.Error())
		assert.Equal(t, test.want, b.String())
	}
}
...
`,
	...
	for _, test := range tests {
		var b bytes.Buffer
		sw := generator.NewSnippetWriter(&b, ctx, "$", "$")
		sw.Do(GenerateSetterForEmbeddedStruct(test.root, test.parent, test.member, test.wrapper))
		assert.NoError(t, sw.Error())
		assert.Equal(t, test.want, b.String())
	}
}
